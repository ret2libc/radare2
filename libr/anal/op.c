...
R_API void r_anal_op_fini (RAnalOp *op) {
	if (!op) return;
	if (((ut64)(size_t)op) == UT64_MAX) return;
	if (((ut64)(size_t)op->mnemonic) == UT64_MAX) return;
...
R_API char *r_anal_op_to_string (RAnal *anal, RAnalOp *op) {
	...
	case R_ANAL_OP_TYPE_CJMP:
	{
		...
		if (bb) {
			...
			snprintf (ret, sizeof (ret), "if (%s) goto 0x%" PFMT64x, "unk", op->jump);
		}
		break;
	}
	case R_ANAL_OP_TYPE_JMP:
